...
- Validate reported checks against this repo's workflow (`./gradlew test` and `./gradlew :app:assembleDebug` when app/build outputs are affected).
...
- Inspect Android app sources/resources (`app/src/main/`), tests (`app/src/test/`), and build scripts (`buildapk.sh`, Gradle files) for drift.
